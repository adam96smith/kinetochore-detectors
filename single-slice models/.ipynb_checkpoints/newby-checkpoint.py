...
parser.add_argument("--postproc", type = str, default = '2')
a = parser.parse_args()
...
def create_model():
    ...
    prev_inputs = tf.keras.Input(shape=(image_size//2, image_size//2, 6))
    
    # Layer 1:
    x11 = layers.Conv2D(3,(9,9),strides=2, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(inputs)
    x11 = nonlinearity(x11)
    x12 = layers.Conv2D(3,(6,6),strides=2, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(inputs)
    x12 = nonlinearity(x12)
    x13 = layers.Conv2D(3,(3,3),strides=1, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(inputs)
    x13 = layers.MaxPool2D()(x13)
    x13 = nonlinearity(x13)
    x1 = layers.Concatenate(axis=3)([x11,x12,x13])
    # x1 = (batch, x//2, y//2, 9) # layer 1 output
    
    # Layer 2:
    x21 = layers.Conv2D(6,(7,7),strides=1, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(x1)
    x21 = nonlinearity(x21)
    x22 = layers.Conv2D(6,(7,7),strides=1, dilation_rate = (2,2), padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(x1)
    rnn_input = nonlinearity(x22)
    x23 = layers.Conv2D(6,(3,3),strides=1, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(x1)
    x23 = nonlinearity(x23)
    x2 = layers.Concatenate(axis=3)([x21,x23,prev_inputs])
    # x2 = (batch, x//2, y//2, 18) # layer 2 output
    # rnn_input = (batch, x//2, y//2, 6)
    
    # RNN layer
    rnn_out = layers.Conv2D(6,(7,7),strides=1, dilation_rate = (3,3), padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(rnn_input)
    # rnn_out = (batch, x//2, y//2, 6)
    
    #Layer 3:
    x3 = layers.Conv2D(1, (5,5), strides=1, padding='SAME', kernel_initializer=tf.random_normal_initializer(0,0.02))(x2)
    main_out = layers.UpSampling2D(size=(2,2), interpolation='bilinear')(x3)
...
def train():
    ...
    for epoch in range(1,epochs+1):
        ...
        for z in z_sample:
            ...
            for t in range(0,image_batch.shape[-1]):
                
                if t == 0:
                    prev_step = tf.zeros(shape = [batch_size,image_size//2,image_size//2,6])
                    
                else:
                    prev_step = rnn_out[-1]
                    
                inputs = tf.slice(image_batch,[0,0,0,t],[batch_size,image_size,image_size,1])
                
                targets = tf.slice(image_batch,[0,0,image_size,t],[batch_size,image_size,image_size,1])
...
def train_plus():
    ...
    for epoch in range(a.pre_epoch+1, a.pre_epoch + epochs + 1):
        ...
        if epoch % 5 == 1:
            snr = 1.5+4.5*np.random.rand()
            data = movie_maker(snr,20)
            z_sample = rd.sample(np.linspace(31,60,40).tolist(),20)
...
def evaluate():
    ...
    for i in range(0,len(filenames)):
        ...
        if arr_type == 'float32':
          input_data = (np.array([sio.loadmat(test_dir + f)['instance'] for f in filenames[i:i+1] if f.endswith('.mat')]).astype(np.float32))
        elif arr_type == 'uint8':
        ...
        prev_rnn_out = np.zeros([image_size//2, image_size//2, in_z, 6]).astype(np.float32)
        ...
        for t in range(0, in_t):
            ...
            for z in range(0,in_z):
                ...
                tf_prev_step = tf.convert_to_tensor(np.reshape(prev_rnn_out[:,:,z,:],[1,image_size//2,image_size//2,6]))
            
                inputs = np.reshape(cv2.resize(input_data[0,:,:,z,t],(image_size, image_size), interpolation=cv2.INTER_NEAREST),[1,image_size,image_size,1])
                tf_inputs = tf.convert_to_tensor(inputs, dtype=tf.float32)

                tf_output, rnn_out = model([tf_inputs,tf_prev_step], training=True)

                prev_rnn_out[:,:,z,:] = rnn_out.numpy()[0,:,:,:]
                out[:,:,z] = tf_output.numpy()[0,:,:,0]
...
#         sio.savemat(output_dir+'/mat/'+filenames[i][0:-4]+'_'+method+'_'+a.postproc+'_outputs.mat', {'instance': outmat})
...
########################################################################################################################### 
        ...
        for i in range(0,movies):
            ...
            for t in range(0,movie.shape[-1]):
                print('Running Model...')
                out = np.zeros([image_size,image_size,movie.shape[2]]).astype(np.float32)
                
                if t == 0:
                    prev_step = tf.zeros(shape = [1,image_size//2,image_size//2,movie.shape[2],6])
                else:
                    prev_step = prev_rnn_out[-1]
                
                tmp = np.zeros([1,image_size//2,image_size//2,movie.shape[2],6]).astype(np.float32)
                for z in range(0,movie.shape[2]):
                    
                    input_slice = tf.convert_to_tensor(np.reshape(movie[:,0:image_size,z,t],[1,image_size,image_size,1]).astype(np.float32))
                    rnn_slice = tf.convert_to_tensor(np.reshape(prev_step[0,:,:,z,:],[1,image_size//2,image_size//2,6]))
                    
                    out_slice, rnn_out = model([input_slice,rnn_slice], training=True)
                    
                    tmp[:,:,:,z,:] = rnn_out.numpy()
                    
                    out[:,:,z] = out_slice.numpy()[0,:,:,0]
                    
                prev_rnn_out.append(tmp)
