...
def create_model():
    ...
    next_obs = tf.keras.Input(shape=(image_size, image_size, 1))  
    inputs = tf.keras.Input(shape=(image_size, image_size, 1))
    prev = tf.keras.Input(shape=(image_size, image_size, 1))
    
    network_input = layers.Concatenate(axis=3)([next_obs, inputs, prev])
    
    x = layers.Conv2D(64,3,strides=1, padding="SAME",kernel_initializer=tf.random_normal_initializer(0,0.02))(network_input)
    ...
    rectified = layers.ReLU()(x)
    output = layers.Conv2DTranspose(1,3,strides=[1,1],padding="SAME", 
            kernel_initializer=tf.random_normal_initializer(0,0.02))(rectified)
    output = tf.keras.activations.sigmoid(output)
    network_layers.append(output)
    
    model = tf.keras.Model(inputs=[next_obs,inputs,prev], outputs=network_layers[-1])
    return model
...
# @tf.function
# def loss_function(outputs,targets):
#     cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(labels=targets, logits=outputs)
#     loss = tf.nn.compute_average_loss(cross_ent)
#     return loss


@tf.function
def train_step(inputs, next_step, prev_step, targets):
    
    with tf.GradientTape() as tape:
        outputs = model([next_step,inputs,prev_step],training=True)
    ...
    for epoch in range(1,epochs+1):
        ...
        for z in z_sample:
            ...
            for i in range(0,image_batch.shape[-1]):
                ...
                if i == 0:
                    next_step = tf.slice(image_batch,[0,0,0,i+1],[batch_size,image_size,image_size,1])
                    inputs = tf.slice(image_batch,[0,0,0,i],[batch_size,image_size,image_size,1])
                    prev_step = tf.zeros(shape = [batch_size,image_size,image_size,1])
                    
                elif i == image_batch.shape[-1]-1:
                    prev_step = tf.slice(image_batch,[0,0,0,i-1],[batch_size,image_size,image_size,1])
                    inputs = tf.slice(image_batch,[0,0,0,i],[batch_size,image_size,image_size,1])
                    next_step = tf.zeros(shape = [batch_size,image_size,image_size,1])
                ...
                loss, output = train_step(inputs,next_step,prev_step,targets)
...
def load_test():
    ...
    return raw_data, raw_data.shape[2]
...
def evaluate():
    ...
    output_dir = './'+a.out_dir+'/'+method+'_'+a.postproc+'/method'+a.postproc+'/'
    ...
    for i in range(0,len(filenames)):
        ...
        for t in range(0,in_t):
            ...
            for z in range(0,in_z):
                ...
                if t == 0:
                    next_step = np.reshape(cv2.resize(input_data[0,:,:,z,t+1],(256,256), interpolation=cv2.INTER_NEAREST),[1,image_size,image_size,1])
                    tf_next_step = tf.convert_to_tensor(next_step, dtype=tf.float32)
                    ...
                    tf_prev_step = tf.zeros(shape = [1,image_size,image_size,1], dtype=tf.float32)
                    
                elif t == in_t-1:
                    prev_step = np.reshape(cv2.resize(input_data[0,:,:,z,t-1],(256,256), interpolation=cv2.INTER_NEAREST),[1,image_size,image_size,1])
                    tf_prev_step = tf.convert_to_tensor(prev_step, dtype=tf.float32)
                    
                    inputs = np.reshape(cv2.resize(input_data[0,:,:,z,t],(256,256), interpolation=cv2.INTER_NEAREST),[1,image_size,image_size,1])
                    tf_inputs = tf.convert_to_tensor(inputs, dtype=tf.float32)
                    
                    tf_next_step = tf.zeros(shape = [1,image_size,image_size,1], dtype=tf.float32)
                ...
                tf_output = model([tf_next_step,tf_inputs,tf_prev_step], training=True)
        ...
        for i in range(0,movies):
            ...
            for t in range(0,movie.shape[-1]):
                ...
                for z in range(0,movie.shape[-2]):
                    ...
                    if t == 0:
                        input_slice = tf.convert_to_tensor(
                            np.reshape(movie[:,0:image_size,z,t],[1,image_size,image_size,1]))
                        prev_slice = tf.zeros(shape=[1,image_size,image_size,1])
                        next_slice = tf.convert_to_tensor(
                            np.reshape(movie[:,0:image_size,z,t+1],[1,image_size,image_size,1]))
                        
                    elif t == movie.shape[-1]-1:
                        
                        input_slice = tf.convert_to_tensor(
                            np.reshape(movie[:,0:image_size,z,t],[1,image_size,image_size,1]))
                        next_slice = tf.zeros(shape=[1,image_size,image_size,1])
                        prev_slice = tf.convert_to_tensor(
                            np.reshape(movie[:,0:image_size,z,t-1],[1,image_size,image_size,1]))
                    ...
                    out_slice = model([next_slice,input_slice,prev_slice], training=True)
